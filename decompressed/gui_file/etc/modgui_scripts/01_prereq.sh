...
move_env_var() {
	if [ ! -f /etc/config/modgui ]; then
		...
		app_entities="xupnp_app voipblock_for_mmpbx voipblock_for_asterisk blacklist_app telstra_webui transmission_webui aria2_webui amule_webui luci_webui adblock_app rsyncd_app speedtest_app adguardhome_app openspeedtest_app wireguard_app l2tpipsec_app openvpn_app tailscale_app"
